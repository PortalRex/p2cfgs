...
DoIncludeScript( "choreo/glados_coop_scenetable_include_manual_dlc1", self.GetScriptScope() )
...
function CoopHubTrack06(){
	GladosPlayVcd(1130) // FIX ME: Need a new glados line here
}
...
function BotDeath(player,dmgtype)
{
	...
	if (curMapName == "mp_coop_lobby_2" || curMapName == "mp_coop_lobby_3"){		
	...
	if ((curMapName == "mp_coop_lobby_2" || curMapName == "mp_coop_lobby_3") && InHub2==1 && InHub2Set==0 )	{
			InHub2Set=1
			GladosPlayVcd(1096)
	}
...
/////////XXXXXX  Building Course
	...
	//DLC functions
		DLC1_GladosCoopOpenExitDoor(player)
	//-----------------------------
...
function CoopReturnHubTrack06(){	// FIX ME
	local tVcd
	tVcd=RandomInt(1277,1279)
	printldebug("THIS IS RANDOM:"+ tVcd)
	GladosPlayVcd(tVcd)
...
function DLC1_GladosCoopMapStart(){
	local curMapName = GetMapName()
	switch (curMapName)
	{
		case "mp_coop_separation_1":
			GladosPlayVcd(-10000)	
			break		
...
function DLC1_GladosCoopOpenExitDoor(player)
{
	local mapname = GetMapName()
	switch (curMapName)
	{
...
function DLC1_mp_coop_tripleaxis_intro()
{
	GladosPlayVcd(-10020)		
}

function DLC1_mp_coop_2paints_1bridge_machine_break()
{
	GladosPlayVcd(-10050)		
}

//On start of night vision gag
function DLC1_mp_coop_paint_crazy_box_lightsout()
{
	GladosPlayVcd(-10145)
}

//On final gantry gate starting to open
function DLC1_mp_coop_paint_crazy_box_gantry_open()
{
	GladosPlayVcd(-10170)
}

//When players are in position for the final boss door
function DLC1_mp_coop_paint_crazy_box_gantry_at_door()
{
	printl("***********************AT DOOR!!!!!!***************************")
}


//On first disassembler malfunction
function DLC1_2Paints_1Bridge_Malfunction()
{
	GladosPlayVcd(-10050)
}

//On land in offices 
function DLC1_2Paints_1Bridge_Offices()
{
	GladosPlayVcd(-10060)
}

//On Seeing office switches 
function DLC1_2Paints_1Bridge_SeeSwitches()
{
	GladosPlayVcd(-10075)
}
...
function DLC1_2Paints_1Bridge_PowerCycle()
{
	GladosPlayVcd(-10080)
}

//On enter white paint 
function DLC1_2Paints_1Bridge_WhitePaintIntro()
{
}

//On exit white paint 
function DLC1_2Paints_1Bridge_WhitePaintOutro()
{
	EntFire( "@relay_disable_exit", "Trigger", "", 0.0 )
	GladosPlayVcd(-10090)
}
...
//---------------------------------------
//AUTOGENERATED DEATH FUNCTION
//---------------------------------------
function DLC1_BotDeath(player,dmgtype)
{
	//player:
	//	1 = orange
	//	2 = blue
	...
	if (curMapName == "mp_coop_separation_1" && (dlc1_deathbits & (1 << 1)) == 0 && dmgtype == 2)
	{
		dlc1_deathbits = dlc1_deathbits + (1 << 1)
		GladosPlayVcd(-11000)
		return
	}
	if (curMapName == "mp_coop_separation_1" && (dlc1_deathbits & (1 << 2)) == 0)
	{
		dlc1_deathbits = dlc1_deathbits + (1 << 2)
		GladosPlayVcd(-11001)
		return
	}
	if (curMapName == "mp_coop_separation_1" && (dlc1_deathbits & (1 << 3)) == 0)
	{
		dlc1_deathbits = dlc1_deathbits + (1 << 3)
		GladosPlayVcd(-11002)
		return
	}
	if (curMapName == "mp_coop_separation_1" && (dlc1_deathbits & (1 << 4)) == 0)
	{
		dlc1_deathbits = dlc1_deathbits + (1 << 4)
		GladosPlayVcd(-11003)
		return
	}
	if (curMapName == "mp_coop_catapult_catch" && (dlc1_deathbits & (1 << 1)) == 0 && (dmgtype == 0 || dmgtype >1000))
	{
		dlc1_deathbits = dlc1_deathbits + (1 << 1)
		GladosPlayVcd(-11004)
		return
	}
	if (curMapName == "mp_coop_catapult_catch" && (dlc1_deathbits & (1 << 2)) == 0 && (dmgtype == 0 || dmgtype >1000))
	{
		dlc1_deathbits = dlc1_deathbits + (1 << 2)
		GladosPlayVcd(-11005)
		return
	}
	if (curMapName == "mp_coop_catapult_catch" && (dlc1_deathbits & (1 << 3)) == 0 && (dmgtype == 0 || dmgtype >1000))
	{
		dlc1_deathbits = dlc1_deathbits + (1 << 3)
		GladosPlayVcd(-11006)
		return
	}
	if (curMapName == "mp_coop_catapult_catch" && (dlc1_deathbits & (1 << 4)) == 0 && (dmgtype == 0 || dmgtype >1000))
	{
		dlc1_deathbits = dlc1_deathbits + (1 << 4)
		GladosPlayVcd(-11007)
		return
	}
	if (curMapName == "mp_coop_catapult_catch" && (dlc1_deathbits & (1 << 5)) == 0 && (dmgtype == 0 || dmgtype >1000))
	{
		dlc1_deathbits = dlc1_deathbits + (1 << 5)
		GladosPlayVcd(-11008)
		return
	}
	if (curMapName == "mp_coop_2paints_1bridge" && (dlc1_deathbits & (1 << 1)) == 0 && (dmgtype == 0 || dmgtype >1000))
	{
		dlc1_deathbits = dlc1_deathbits + (1 << 1)
		GladosPlayVcd(-11009)
		return
	}
	if (curMapName == "mp_coop_2paints_1bridge" && (dlc1_deathbits & (1 << 2)) == 0 && (dmgtype == 0 || dmgtype >1000))
	{
		dlc1_deathbits = dlc1_deathbits + (1 << 2)
		GladosPlayVcd(-11010)
		return
	}
	if (curMapName == "mp_coop_2paints_1bridge" && (dlc1_deathbits & (1 << 3)) == 0 && (dmgtype == 0 || dmgtype >1000))
	{
		dlc1_deathbits = dlc1_deathbits + (1 << 3)
		GladosPlayVcd(-11011)
		return
	}
	if (curMapName == "mp_coop_2paints_1bridge" && (dlc1_deathbits & (1 << 4)) == 0 && (dmgtype == 0 || dmgtype >1000))
	{
		dlc1_deathbits = dlc1_deathbits + (1 << 4)
		GladosPlayVcd(-11012)
		return
	}
	if (curMapName == "mp_coop_bridge_catch" && (dlc1_deathbits & (1 << 1)) == 0 && (dmgtype == 0 || dmgtype >1000))
	{
		dlc1_deathbits = dlc1_deathbits + (1 << 1)
		GladosPlayVcd(-11013)
		return
	}
	if (curMapName == "mp_coop_bridge_catch" && (dlc1_deathbits & (1 << 2)) == 0 && (dmgtype == 0 || dmgtype >1000))
	{
		dlc1_deathbits = dlc1_deathbits + (1 << 2)
		GladosPlayVcd(-11014)
		return
	}
	if (curMapName == "mp_coop_bridge_catch" && (dlc1_deathbits & (1 << 3)) == 0)
	{
		dlc1_deathbits = dlc1_deathbits + (1 << 3)
		GladosPlayVcd(-11015)
		return
	}
	if (curMapName == "mp_coop_bridge_catch" && (dlc1_deathbits & (1 << 4)) == 0)
	{
		dlc1_deathbits = dlc1_deathbits + (1 << 4)
		GladosPlayVcd(-11016)
		return
	}
	if (curMapName == "mp_coop_laser_tbeam" && (dlc1_deathbits & (1 << 1)) == 0)
	{
		dlc1_deathbits = dlc1_deathbits + (1 << 1)
		GladosPlayVcd(-11017)
		return
	}
	if (curMapName == "mp_coop_laser_tbeam" && (dlc1_deathbits & (1 << 2)) == 0)
	{
		dlc1_deathbits = dlc1_deathbits + (1 << 2)
		GladosPlayVcd(-11018)
		return
	}
	if (curMapName == "mp_coop_laser_tbeam" && (dlc1_deathbits & (1 << 3)) == 0)
	{
		dlc1_deathbits = dlc1_deathbits + (1 << 3)
		GladosPlayVcd(-11019)
		return
	}
	if (curMapName == "mp_coop_paint_rat_maze" && (dlc1_deathbits & (1 << 1)) == 0 && dmgtype == 1)
	{
		dlc1_deathbits = dlc1_deathbits + (1 << 1)
		GladosPlayVcd(-11020)
		return
	}
	if (curMapName == "mp_coop_paint_rat_maze" && (dlc1_deathbits & (1 << 2)) == 0 && dmgtype == 1)
	{
		dlc1_deathbits = dlc1_deathbits + (1 << 2)
		GladosPlayVcd(-11021)
		return
	}
	if (curMapName == "mp_coop_paint_rat_maze" && (dlc1_deathbits & (1 << 3)) == 0 && dmgtype == 1)
	{
		dlc1_deathbits = dlc1_deathbits + (1 << 3)
		GladosPlayVcd(-11022)
		return
	}
	if (curMapName == "mp_coop_paint_rat_maze" && (dlc1_deathbits & (1 << 10)) == 0)
	{
		dlc1_deathbits = dlc1_deathbits + (1 << 10)
		GladosPlayVcd(-11023)
		return
	}
	if (curMapName == "mp_coop_paint_rat_maze" && (dlc1_deathbits & (1 << 11)) == 0)
	{
		dlc1_deathbits = dlc1_deathbits + (1 << 11)
		GladosPlayVcd(-11024)
		return
	}
	if (curMapName == "mp_coop_paint_rat_maze" && (dlc1_deathbits & (1 << 12)) == 0)
	{
		dlc1_deathbits = dlc1_deathbits + (1 << 12)
		GladosPlayVcd(-11025)
		return
	}
	if (curMapName == "mp_coop_paint_rat_maze" && (dlc1_deathbits & (1 << 13)) == 0)
	{
		dlc1_deathbits = dlc1_deathbits + (1 << 13)
		GladosPlayVcd(-11026)
		return
	}
}
//---------------------------------------
//END OF AUTOGENERATED DEATH FUNCTION
//---------------------------------------

//--------------------------------------------------------------------------------------------------------------
//END OF DLC 1 FUNCTIONS
//--------------------------------------------------------------------------------------------------------------
