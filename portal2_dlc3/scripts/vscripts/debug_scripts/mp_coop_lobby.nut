...
function OpenHUBAreas()
{
	...
	local bCameFromLastDLCMap = GetCameFromLastDLCMap();

	// If we transitioned from the last DLC map AND the num highest branch tells us we've not completed everything
	if( bCameFromLastDLCMap && numHighestActiveBranch != 7 )
	{
		// Remove the glados return speech relay so glados wont jeer the players for quitting
		// since they're returning to the hub after completing the DLC
		EntFire( "relay_ret_glados", "kill" )
	}

	if( GetHaveSeenDLCTubesReveal() == false )
	{
		EntFire( "relay_reveal_extra", "Enable", 0, 0 )
		SetHaveSeenDLCTubesReveal()
	}
	else
	{
		EntFire( "relay_quick_glados_extra", "Enable", 0, 0 )
	}
